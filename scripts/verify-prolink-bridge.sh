...
if [[ -z "${JAVA_HOME:-}" || ! -x "$JAVA_HOME/bin/java" ]]; then
  echo "ERROR: OpenJDK 21 is required to build the Pro DJ Link bridge." >&2
  exit 1
fi
...
if [[ "$java_version" != *'21.'* ]]; then
  echo "ERROR: Pro DJ Link bridge build requires Java 21, found: $java_version" >&2
  exit 1
fi
...
echo "Pro DJ Link bridge verification passed."
