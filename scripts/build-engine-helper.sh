...
/usr/libexec/PlistBuddy -c "Add :NSRemovableVolumesUsageDescription string Lumi reads trusted Rekordbox USB media to synchronize playlists, analysis, cue points, and playable track locations. Rekordbox data is never changed; Lumi may add one hidden identity marker to keep identical USB disks separate." "$engine_info_plist"
