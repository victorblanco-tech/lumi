...
impl ReconcilePreview {
    ...
    pub fn between(
        ...
        let rebase_ambiguities = current
            ...
            .filter(|phrase| {
                u64::from(phrase.end_beat()) * u64::from(source_total_beats)
                    % u64::from(current.total_beats())
                    != 0
            })
...
pub fn reconcile_timeline(
    ...
    let phrases = match strategy {
        ReconcileStrategy::KeepLumi => {
            if current.total_beats() != source_total_beats {
                return Err(ReconcileError::KeepLumiDurationChanged);
            }
            current.phrases().to_vec()
        }
        ReconcileStrategy::Rebase => rebase_phrases(current, source_total_beats)?,
        ReconcileStrategy::Merge(choices) => {
            merge_phrases(current, source_phrases, choices, source_total_beats)?
        }
...
fn rebase_phrases(
    ...
    if source_total_beats == 0 || source_total_beats < current.phrases().len() as u32 {
        return Err(ReconcileError::InsufficientBeats);
    }
    ...
    for (index, phrase) in current.phrases().iter().enumerate() {
        ...
        let end = if index + 1 == phrase_count {
            ...
            let numerator = u64::from(phrase.end_beat()) * u64::from(source_total_beats);
            let denominator = u64::from(current.total_beats());
            let rounded = (numerator + denominator / 2) / denominator;
            u32::try_from(rounded)
                .map_err(|_| ReconcileError::ArithmeticOverflow)?
                .clamp(previous + 1, source_total_beats - remaining)
        };
        result.push(
...
fn merge_phrases(
    ...
    let preview = ReconcilePreview::between(current, source, source_total_beats);
    ...
    validate_coverage(&merged, source_total_beats)?;
    Ok(merged)
}

fn validate_coverage(phrases: &[PhraseInstance], total_beats: u32) -> Result<(), ReconcileError> {
    let mut previous = 0;
    for phrase in phrases {
        if phrase.start_beat() != previous || phrase.end_beat() <= phrase.start_beat() {
            return Err(ReconcileError::NonContiguousMerge);
        }
        previous = phrase.end_beat();
    }
    if previous != total_beats {
        return Err(ReconcileError::NonContiguousMerge);
    }
    Ok(())
}
    ...
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::KeepLumiDurationChanged => {
                formatter.write_str("Keep Lumi requires the same beat duration")
            }
            Self::InsufficientBeats => {
                formatter.write_str("source has too few beats for the timeline")
            }
